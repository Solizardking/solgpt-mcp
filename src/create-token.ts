...
import { generateImageForToken } from "./generate-image.js";
...
export async function createToken(
  ...
  try {
    ...
    if (generateImagePrompt) {
      console.error(`Generating image with prompt: ${generateImagePrompt}`);
      const imageResult = await generateImageForToken(generateImagePrompt, name);
      if (imageResult.success && imageResult.imagePath) {
        imageUrl = imageResult.imagePath;
        console.error(`Using generated image: ${imageResult.imagePath}`);
      } else {
